...
##
# RubyZen::Parser::C attempts to parse C extension files.  It looks for
...
#
# Here RubyZen will determine from the rb_define_method line that there's a
...
#
# In addition, RubyZen assumes by default that the C method implementing a
...
class RubyZen::Parser::C < RubyZen::Parser

  parse_files_matching(/\.(?:([CcHh])\1?|c([+xp])\2|y)\z/)

  # include RubyZen::Text
  ...
  ##
  # Prepares for parsing a C file.  See RubyZen::Parser#initialize for details on
  # the arguments.

  # def initialize top_level, file_name, content, options, stats
  def initialize(content, engine)
    super

    @options = OpenStruct.new(tab_width: 8)
    @known_classes = RubyZen::KNOWN_CLASSES.dup
  ...
  def deduplicate_method_name class_obj, method_name # :nodoc:
    return unless
      ...
      method = class_obj.method_list[method_name]
  ...
  def do_includes
    @content.scan(/rb_include_module\s*\(\s*(\w+?),\s*(\w+?)\s*\)/) do |c,m|
      ...
      comment = RubyZen::Comment.new '', @top_level
      incl = cls.add_include RubyZen::Include.new(m, comment)
  ...
  def find_alias_comment class_name, new_name, old_name
    ...
    RubyZen::Comment.new($1 || '', @top_level)
  end
  ...
  def find_attr_comment var_name, attr_name, read = nil, write = nil
    ...
    RubyZen::Comment.new comment, @top_level
  end
  ...
  def find_body class_name, meth_name, meth_obj, file_content, quiet = false
    ...
    case type
    when :func_def
      # comment = RubyZen::Comment.new args[0], @top_level
      comment = RubyZen::Comment.new args[0]
      body = args[1]
      offset, = args[2]
  ...
  def find_class_comment class_name, class_mod
    ...
    comment = RubyZen::Comment.new comment, @top_level
    comment.normalize
  ...
  def find_override_comment class_name, meth_obj
    ...
    # RubyZen::Comment.new comment, @top_level
    RubyZen::Comment.new comment
  end

  ##
  # Creates a new RubyZen::Attr +attr_name+ on class +var_name+ that is either
  # +read+, +write+ or both
  ...
  def handle_attr(var_name, attr_name, read, write)
    ...
    attr = RubyZen::Attr.new '', name, rw, comment
  ...
  ##
  # Creates a new RubyZen::NormalClass or RDoc::NormalModule based on +type+
  ...
  # RubyZen.  Values may include quotes and escaped colons (\:).
  ...
  def handle_constants(type, var_name, const_name, definition)
    ...
    if type.downcase == 'const' then
      ...
      if no_match and no_match.empty? then
        ...
        new_comment = RubyZen::Comment.new new_comment, @top_level

        con = RubyZen::Constant.new const_name, new_definition, new_comment
      else
        con = RubyZen::Constant.new const_name, definition, comment
      end
    else
      con = RubyZen::Constant.new const_name, definition, comment
    end
  ...
  ##
  # Adds an RubyZen::AnyMethod +meth_name+ defined on a class or module assigned
  ...
  def handle_method(type, var_name, meth_name, function, param_count,
    ...
    if class_obj then
      ...
      if body
        meth_obj.parameters = if p_count < -1 then # -2 is Array
                            '(*args)'
  ...
  ##
  # Loads the variable map with the given +name+ from the RubyZen::Store, if
  # present.
  ...
  # from a C file and returns an RubyZen::TopLevel for this file
