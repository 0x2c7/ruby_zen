...
module RubyZen::Text
  ...
  # MARKUP_FORMAT = {
  #   'markdown' => RubyZen::Markdown,
  #   'rdoc'     => RubyZen::Markup,
  #   'rd'       => RubyZen::RD,
  #   'tomdoc'   => RubyZen::TomDoc,
  # }
  #
  # MARKUP_FORMAT.default = RubyZen::Markup
  ...
  def expand_tabs text
    ...
    text.each_line do |line|
      nil while line.gsub!(/(?:\G|\r)((?:.{8})*?)([^\t\r\n]{0,7})\t/) do
        r = "#{$1}#{$2}#{' ' * (8 - $2.size)}"
        r = RubyZen::Encoding.change_encoding r, text.encoding
        r
      end
  ...
  def markup text
    ...
    if locale
      i18n_text = RubyZen::I18n::Text.new(text)
  ...
  ##
  # Normalizes +text+ then builds a RubyZen::Markup::Document from it

  def parse text, format = 'rdoc'
    return text if RubyZen::Markup::Document === text
    return text.parse if RubyZen::Comment === text

    text = normalize_comment text # TODO remove, should not be necessary

    return RubyZen::Markup::Document.new if text =~ /\A\n*\z/
  ...
  def snippet text, limit = 100
    ...
    RubyZen::Markup::ToHtmlSnippet.new(options, limit).convert document
  end
  ...
  def strip_stars text
    ...
    space = ' '
    space = RubyZen::Encoding.change_encoding space, encoding if encoding
    ...
    empty = ''
    empty = RubyZen::Encoding.change_encoding empty, encoding if encoding
    text.gsub(/^\s+$/, empty)
  end
  ...
  def to_html text
    ...
    encoded = RubyZen::Text::TO_HTML_CHARACTERS[text.encoding]
